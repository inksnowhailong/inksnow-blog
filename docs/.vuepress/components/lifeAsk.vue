 ...
 *
 * 这是 life 页 AI 对话的唯一外壳形态。定它之前跑过三版对照：
 *   就地浮层（本版）  点哪弹哪，一次一个对象，不留历史
 *   底部指令条        单一入口常驻底部，对象做成 chip，可一次作用于多个
 *   右侧抽屉          对象详情与多轮对话上下并列，页面收窄而非被遮挡
 * 选了就地浮层。要再改形态先把这三条重新比一遍，别只盯着单个页面顺不顺手。
 *
 * 面板本体在 lifeAskPanel.vue，与 lifeChat 的常驻态共用同一棵标记树，
 * 这里只负责把它塞进浮层并算锚定位置。
 */
      ...
      >
        ...
        >
          <p
            v-if="reply"
            data-alt="ask-reply"
            class="whitespace-pre-wrap rounded-lg bg-slate-50 px-3 py-2.5 text-sm leading-relaxed text-slate-600 dark:bg-slate-700/50 dark:text-slate-300 sm:px-2.5 sm:py-2 sm:text-xs"
          >
            {{ reply }}
          </p>
        </LifeAskPanel>
