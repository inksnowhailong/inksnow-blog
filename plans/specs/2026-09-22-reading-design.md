...
## 追加 · 读书回到打卡卡，独占一行、置顶（2026-09-23，第三版，用户拍板）

用户原话："读书还是放到每日任务项里面去，独占一行"；"这种多余文案不要"；"展示最近读书都有什么书，点击快速记录内容，可以添加更多读书"；"读书最上面，其他三个下面"。

```
┌ 每日四项                                      ‹ 9/23 周三 › ┐
│ ┌ 读书 ─────────────────────────────────── 2 分 ✧ ┐        │ ← 第一行，独占整行（sm:col-span-2）
│ │ 15/30 分钟                         +15  +30  ◎  │        │
│ │ 《穷查理宝典》 《人生十二法则》  ＋               │        │ ← 在读的书 chip，按 lastLogOn 倒序；「＋」在最后
│ │ 穷查理宝典 › [读到什么……………………………] (记下)    │        │ ← 点 chip 展开这一条；记完收起；再点 chip 收起
│ └───────────────────────────────────────────────┘        │
│ ┌ 英文技术阅读 ┐ ┌ AI 控制能力主线 ┐                        │
│ ┌ 数据库       ┐                                            │
└────────────────────────────────────────────────────────────┘
...
- 组件 `lifeReadingRow.vue`（由 `lifeReadingCard.vue` 改来，删掉旧文件）：props `daily`（pinned 那条每日项）、`books`、`busy`、`api`、`isToday`；emits `punch(minutes)`、`clear`、`changed`、`open(book)`。
- 打卡格：pinned 项渲染为第一行 `LifeReadingRow`，其余项照旧两列；`dailyTitle` 重新把 pinned 计入（回到「每日四项」）。
- **无多余文案**：没有在读书时 chip 行只有「＋」；展开态只有输入条和「记下」；不出现"还没记书 / 先记一本"之类的提示。
- 「＋」点了就地变成书名输入（`LifeAskBar mode=note` 或原生 input，Enter 提交，Esc/失焦收起），提交后新书自动成为展开态。
- chip 点开 → 展开一条 `LifeAskBar mode=note`（占位「读到什么，想到什么」），Ctrl+Enter 记下 → `POST /life/books/:id/logs` → 收起、`emit('changed')`。chip 右侧一个小「›」按钮（`data-alt="reading-open"`）→ `emit('open', book)` 开现有书弹窗（看全部笔记/读完/删书）。
- 读完的书不在 chip 行；读完段与「记一本书」输入条不再单独存在（加书走「＋」，看读完走书弹窗？——不，读完列表没有入口就等于没了）：在 chip 行末尾「＋」之后加一个灰字「读完 N」文字按钮（N>0 才显示），点开一个只读小列表弹窗复用 `lifeBookModal` 的列表？太重——直接点开第一本读完书的弹窗没意义。**裁定：读完 N 点开一个轻量下拉（chip 行下方展开读完书名列表，每行可点开书弹窗）**。
- 左列独立读书卡删除；`lifeBookRow.vue` 若无人用则删。
- 验收：打卡卡第一行是读书整行，下面三项两列；无在读书时读书行只有分钟区和「＋」；点 chip 展开输入、记下后收起并进书弹窗时间线；375 宽不横向滚动、无报错。
